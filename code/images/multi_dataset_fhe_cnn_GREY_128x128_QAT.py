...
# ===========================
AVAILABLE_DATASETS = {
    'pneumoniamnist': {'name': 'PneumoniaMNIST', 'classes': 2, 'epochs': 10},
    'octmnist': {'name': 'OCTMNIST', 'classes': 4, 'epochs': 20},
    'breastmnist': {'name': 'BreastMNIST', 'classes': 2, 'epochs': 10},
    'tissuemnist': {'name': 'TissueMNIST', 'classes': 8, 'epochs': 25},
    'organamnist': {'name': 'OrganAMNIST', 'classes': 11, 'epochs': 25},
    'organcmnist': {'name': 'OrganCMNIST', 'classes': 11, 'epochs': 20},
    'organsmnist': {'name': 'OrganSMNIST', 'classes': 11, 'epochs': 20},
}
...
FHE_CONFIGURATIONS = []
for bit_width in range(4, 9):  # 4, 5, 6, 7, 8
...
class cnn128_qat_basico(nn.Module):
    """
    Quantization-Aware Training (QAT) CNN for 128x128 grayscale medical images.    
    Architecture:
        - Input quantization
        - Conv Block 1: 128x128x1 -> 32x32x8 (stride=4)
        - Conv Block 2: 32x32x8 -> 8x8x12 (stride=4)
        - Fully connected: 768 -> 24 -> num_classes
    """
...
def get_model_path(data_flag, bit_width):
    return os.path.join(MODELS_DIR, f'model_{data_flag}_bw{bit_width}.pth')
...
def load_trained_model(data_flag, bit_width, num_classes):
    ...
    try:
        model = cnn128_qat_basico(num_classes=num_classes, bit_width=bit_width)
...
def load_dataset(data_flag, batch_size=BATCH_SIZE):
    ...
    if sample_img.shape[0] != 1:
        raise ValueError(f"ERROR: {data_flag} is not grayscale! It has {sample_img.shape[0]} channels")
...
def get_save_path(data_flag):
    return f'results_GREY_128x128_qat_{data_flag}.csv'
...
def save_clear_results(data_flag, bit_width, acc, f1, save_path='results_GREY_128x128_qat_clear.csv'):
    if not os.path.isfile(save_path):
        with open(save_path, mode='w', newline='') as f:
            writer = csv.writer(f)
...
if __name__ == '__main__':
    try:
        ...
        print(f'\n{"="*60}')
        print(f'Device: {device} (QAT with Brevitas - BASIC MODEL)')
        ...
        # Show epochs per dataset
        print("\nConfigured epochs per dataset:")
        for data_flag, info in AVAILABLE_DATASETS.items():
            print(f"  {info['name']:20s}: {info['epochs']:2d} epochs")
            ...
            try:
                ...
                for bw_idx, bit_width in enumerate(unique_bit_widths, start=1):
                    ...
                    if loaded is not None:
                        ...
                        print(f"QAT Evaluation (bit_width={bit_width}) | Accuracy: {acc:.4f} | F1 macro: {f1:.4f}")
