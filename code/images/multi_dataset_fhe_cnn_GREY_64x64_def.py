...
AVAILABLE_DATASETS = {
    ...
    'chestmnist': {'name': 'ChestMNIST', 'classes': 14, 'epochs': 25},
...
def load_dataset(data_flag, batch_size=BATCH_SIZE):
    ...
    print(f"  Train: {len(train_dataset)} muestras (100%)")
    print(f"  Val: {len(val_dataset)} muestras (100%)")
    print(f"  Test: {len(test_dataset)} muestras (100%)")
...
def save_traditional_results(data_flag, acc, f1, save_path='results_64x64_traditional.csv'):
    if not os.path.isfile(save_path):
        with open(save_path, mode='w', newline='') as f:
            writer = csv.writer(f)
...
if __name__ == '__main__':
    try:
        ...
        print(f'Datasets a procesar: {len(AVAILABLE_DATASETS)}')
        print(f'Configuraciones FHE por dataset: {len(FHE_CONFIGURATIONS)}')
        print(f'{"="*60}')
        ...
        for dataset_idx, (data_flag, dataset_info) in enumerate(AVAILABLE_DATASETS.items(), start=1):
            ...
            try:
                ...
                # Entrenamiento
                print("\n" + "="*60)
                print(f"FASE 1: ENTRENAMIENTO ({epochs} epochs)")
                print("="*60)
                ...
                save_traditional_results(data_flag, acc, f1)
                ...
                for cfg_idx, (n_bits, rounding_threshold, p_error) in enumerate(FHE_CONFIGURATIONS, start=1):
                    ...
                    print(f"\n=== EXPERIMENTO {cfg_idx}/{len(FHE_CONFIGURATIONS)} ===")
                    ...
                    if 'q_model' in locals():
                        del q_model
                    clear_memory()
                    print(f"✓ Experimento {cfg_idx} completado\n")
                
                print(f"\n✓ Dataset {dataset_info['name']} completado")
                
            except Exception as e:
                print(f"✗ Error en dataset {data_flag}: {e}")
                import traceback
                traceback.print_exc()
                continue
            
            finally:
                # Limpiar memoria entre datasets
                if 'model' in locals():
                    del model
        ...
        print("\n" + "="*60)
        print("✓ TODOS LOS DATASETS Y EXPERIMENTOS COMPLETADOS")
        print("="*60)
